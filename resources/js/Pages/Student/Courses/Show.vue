...
function handleStart() {
  router.post(route('courses.start', props.course.id))
}
  ...
  <student-layout>
    ...
    <div class="py-24 h-full sm:py-32">
      <div class="max-w-7xl mx-auto">
        <div>
          <div>
            <img v-if="course.image_url" class="h-48 w-48 md:h-56 md:w-56 rounded-full" :src="course.image_url" alt=""/>
          ...
          <div class="flex-1 ml-3 text-center">
            <p class="mt-2 text-3xl font-heading font-bold tracking-tight text-gray-900 sm:text-4xl text-center">
              {{ course.name }}</p>
            <primary-button v-if="!course.enrolled && topics.length > 0" class="mt-4 w-1/6" @click="handleStart()">
              Start Course
            </primary-button>
        ...
        <template v-for="title in Array.from(new Set(topics.map(topic => topic.title_id)))" :key="title">
          <div
            ...
            <div class="p-8 px-6 pt-4 pb-4 border-b dark:border-gray-700">
              <h3 class="text-lg font-medium text-gray-800 dark:text-gray-300 hover:underline w-fit font-heading">
                ...
                <template v-for="topic in topics" :key="topic.id">
                  <div
                    ...
                    <div class="flex items-center" @click="handleView(topic, false)">
                      ...
                      <p class="ml-3 text-gray-800 text-lg dark:text-gray-300 font-heading">{{ topic.title }}</p>
                    </div>
                    <check-circle-icon v-if="course.enrolled" class="text-white w-6 h-6"/>
                  </div>
                  <div v-if="topic.assessments.length > 0" v-for="assessment in topic.assessments"
                       :key="assessment.id"
                    ...
                    <PuzzlePieceIcon v-else
                                     class="text-gray-800 dark:text-gray-300"/>
                    <p class="ml-3 text-gray-800 text-lg dark:text-gray-300 font-heading">{{ assessment.title }}</p>
                  </div>
