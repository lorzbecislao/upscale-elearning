<template>
    <div class="py-4 flex items-center justify-between">
        ...
        <div class="flex-1 flex justify-between sm:hidden">
            <a class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white hover:text-gray-500 focus:outline-none focus:ring-blue-200 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 cursor-pointer"
               @click="previousPage($parent)">
                Previous
            </a>
            <a class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white hover:text-gray-500 focus:outline-none focus:ring-blue-200 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 cursor-pointer"
        ...
        <div class="hidden sm:flex-1 sm:flex sm:justify-end">
            <div>
                <nav class="relative z-0 inline-flex shadow-sm">
                    ...
                    <a v-for="page in group.left"
                       :class="[pager.class.distributable, (page === pagination.current_page ? '!bg-indigo-50' : '')]"
                       @click="toPage($parent, page)">
                        <span v-text="page"></span>
                    </a>
                    <a :class="pager.class.distributable + (pagination.total_pages > 6 ? ' inline-block' : ' hidden')">
                        <span>...</span>
                    </a>
                    <a v-for="page in group.right"
                       :class="[pager.class.distributable, (page === pagination.current_page ? '!bg-indigo-50' : '')]"
                       @click="toPage($parent, page)">
                        <span v-text="page"></span>
                    </a>
